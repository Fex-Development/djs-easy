...
class ButtonMenu {
     ...
     */
    start() {
        if (!this.pages[0]) return new Error("[djs-easy] You are trying to start an empty Menu. Please retry with at least one page in it.");
        ...
        if (this.channel.type === 'dm') {
            ...
            .catch(() => console.log("[djs-easy] The User you are trying to send the Menu to has his DM blocked."));
        } else {
            if (channelPermissions.some(p => !this.channel.permissionsFor(this.channel.client.user.id).has(p))) return new Error("[djs-easy] Your bot does not have one of the following permissions: " + channelPermissions.join(" | "));
            this.channel.send(this.current.toObject())
            .then(message => {
                this.message = message;
                this.awaitButtons();
            })
            .catch(err => console.log("[djs-easy] An error has occured while trying to send the Menu into the desired channel.\n", err));
        }
