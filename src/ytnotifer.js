...
async function ytNotify(client, db, options = []) {
  ...
  if (!chid)
    throw new Error(
      "EMPTY_CHANNEL_ID. You didnt specify a channel id. Go to https://discord.gg/NkvyxzTaNr to get support"
    );
  if (!options.ytID && !options.ytURL)
    throw new Error(
      "EMPTY_YT_CHANNEL_ID & EMPTY_YT_CHANNEL_URL. You didnt specify a channel id. Go to https://discord.gg/NkvyxzTaNr to get support"
    );
  try {
